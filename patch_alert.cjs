...
const replacement = `                        onClick={async () => {
                          try {
                            ...
                            if (e.code === 'auth/user-not-found' || e.message?.includes('not found')) {
                              setErrorMsg('هذا الحساب جديد ولم يقم بتسجيل الدخول بعد. يتم تعيين كلمة المرور لأول مرة بمجرد أن يقوم المستخدم بتسجيل الدخول لحسابه بأي كلمة مرور يختارها.');
                            } else {
