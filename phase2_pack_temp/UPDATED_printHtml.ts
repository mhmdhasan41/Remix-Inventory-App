...
export async function exportToPDF(data: PrintData & { filename: string; orientation?: 'portrait' | 'landscape' }) {
  ...
  pageNodes.forEach((p, pIdx) => {
    ...
    // Remove orphaned section titles
    const titles = Array.from(p.querySelectorAll('.section-title')) as HTMLElement[];
    titles.forEach(t => {
      const next = t.nextElementSibling;
      if (!next || (!next.classList.contains('table-container') && !next.classList.contains('notes-container'))) {
        t.remove();
      }
    });
